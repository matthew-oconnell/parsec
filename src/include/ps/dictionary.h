...
struct Value {
    ...
    Value(const std::vector<int>& v_) {
        std::vector<Value> L;
        for (auto x : v_) L.emplace_back(int64_t(x));
        v = std::move(L);
    }
    Value(const std::vector<double>& v_) {
        std::vector<Value> L;
        for (auto x : v_) L.emplace_back(x);
        v = std::move(L);
    }
    Value(const std::vector<std::string>& v_) {
        std::vector<Value> L;
        for (auto const& x : v_) L.emplace_back(x);
        v = std::move(L);
    }
    Value(const std::vector<bool>& v_) {
        std::vector<Value> L;
        for (auto x : v_) L.emplace_back(x);
        v = std::move(L);
    }
    ...
    const std::shared_ptr<Dictionary>& asDict() const {
        return std::get<std::shared_ptr<Dictionary>>(v);
    }
    ...
    // compatibility: return an int corresponding to Dictionary::TYPE for tests that call .type() on
    // parse_json result
    int type() const;
...
struct Dictionary {
    ...
    Dictionary& operator=(const std::vector<Dictionary>& v) {
        data.clear();  // store as scalar list of Values constructed from dictionaries
        std::vector<Value> L;
        L.reserve(v.size());
    ...
    bool operator==(const Dictionary& rhs) const {
        return data == rhs.data && scalar == rhs.scalar;
    }
    ...
    std::string asString() const {
        if (scalar && scalar->isString()) return scalar->asString();
        throw std::runtime_error("not a string");
    }
    int asInt() const {
        if (scalar && scalar->isInt()) return static_cast<int>(scalar->asInt());
        throw std::runtime_error("not an int");
    }
    double asDouble() const {
        if (scalar && scalar->isDouble()) return scalar->asDouble();
        throw std::runtime_error("not a double");
    }
    bool asBool() const {
        if (scalar && scalar->isBool()) return scalar->asBool();
        throw std::runtime_error("not a bool");
    }
    ...
    bool isValueObject() const {
        return scalar.has_value() and not scalar->isList() and not scalar->isDict();
    }
...
inline std::string Value::to_string() const {
    ...
    if (isDict()) {
        const auto& D = asDict();
        return D ? D->dump() : std::string("null");
    }
    return "<unknown>";
}
...
inline std::vector<int> Value::asInts() const {
    if (isList()) {
        std::vector<int> out;
        for (auto const& e : asList()) out.push_back(e.isInt() ? static_cast<int>(e.asInt()) : 0);
        return out;
    }
...
inline std::vector<double> Value::asDoubles() const {
    if (isList()) {
        ...
        for (auto const& e : asList())
            out.push_back(e.isDouble() ? e.asDouble()
                                       : (e.isInt() ? static_cast<double>(e.asInt()) : 0.0));
        return out;
    }
...
inline std::vector<std::string> Value::asStrings() const {
    if (isList()) {
        std::vector<std::string> out;
        for (auto const& e : asList()) out.push_back(e.isString() ? e.asString() : std::string());
        return out;
    }
...
inline std::vector<bool> Value::asBools() const {
    if (isList()) {
        std::vector<bool> out;
        for (auto const& e : asList()) out.push_back(e.isBool() ? e.asBool() : false);
        return out;
    }
...
inline std::vector<Dictionary> Value::asObjects() const {
    if (isList()) {
        ...
        for (auto const& e : asList())
            if (e.isDict()) out.push_back(*e.asDict());
        return out;
    }
...
inline Value& Value::operator[](const std::string& k) {
    if (!isDict()) {
        std::shared_ptr<Dictionary> p = std::make_shared<Dictionary>();
        v = p;
    }
...
inline std::vector<int> Dictionary::asInts() const {
    if (scalar && scalar->isList()) {
        std::vector<int> out;
        for (auto const& e : scalar->asList())
            out.push_back(e.isInt() ? static_cast<int>(e.asInt()) : 0);
        return out;
    }
    throw std::runtime_error("not an int list");
}
...
inline std::vector<std::string> Dictionary::asStrings() const {
    if (scalar && scalar->isList()) {
        std::vector<std::string> out;
        for (auto const& e : scalar->asList())
            out.push_back(e.isString() ? e.asString() : std::string());
        return out;
    }
    throw std::runtime_error("not a string list");
}
inline std::vector<bool> Dictionary::asBools() const {
    if (scalar && scalar->isList()) {
        std::vector<bool> out;
        for (auto const& e : scalar->asList()) out.push_back(e.isBool() ? e.asBool() : false);
        return out;
    }
    throw std::runtime_error("not a bool list");
}
...
inline Dictionary Dictionary::overrideEntries(const Value& cfg) const {
    if (cfg.isDict()) return overrideEntries(*cfg.asDict());
    throw std::runtime_error("not an object");
}
inline Dictionary Dictionary::merge(const Value& cfg) const {
    if (cfg.isDict()) return merge(*cfg.asDict());
    throw std::runtime_error("not an object");
}
...
inline std::string Value::dump(int indent, bool compact) const {
    if (isDict()) {
        const auto& p = asDict();
        return p ? p->dump(indent, compact) : std::string("null");
    }
    return to_string();
}
...
inline std::vector<std::string> Value::keys() const {
    if (isDict()) {
        const auto& p = asDict();
        return p ? p->keys() : std::vector<std::string>{};
    }
    throw std::runtime_error("not an object");
}
inline bool Value::has(const std::string& k) const {
    if (isDict()) {
        const auto& p = asDict();
        return p ? p->has(k) : false;
    }
    return false;
}
inline int Value::count(const std::string& k) const {
    if (isDict()) {
        const auto& p = asDict();
        return p ? p->count(k) : 0;
    }
    return 0;
}
inline bool Value::contains(const std::string& k) const { return has(k); }

inline Dictionary Value::overrideEntries(const Dictionary& cfg) const {
    if (isDict()) {
        const auto& p = asDict();
        return p ? p->overrideEntries(cfg) : Dictionary();
    }
    throw std::runtime_error("not an object");
}
inline Dictionary Value::merge(const Dictionary& cfg) const {
    if (isDict()) {
        const auto& p = asDict();
        return p ? p->merge(cfg) : Dictionary();
    }
    throw std::runtime_error("not an object");
}

inline Dictionary Value::overrideEntries(const Value& cfg) const {
    if (cfg.isDict()) return overrideEntries(*cfg.asDict());
    throw std::runtime_error("not an object");
}
inline Dictionary Value::merge(const Value& cfg) const {
    if (cfg.isDict()) return merge(*cfg.asDict());
    throw std::runtime_error("not an object");
}
