...
static std::optional<std::string> validate_node(const Value& data,
                                                const Dictionary& schema_root,
                                                const Dictionary& schema_node,
                                                const std::string& path);
static std::optional<std::string> validate_node(const Dictionary& dataObj,
                                                const Dictionary& schema_root,
                                                const Dictionary& schema_node,
                                                const std::string& path);
...
static std::optional<std::string> check_numeric_constraints(const Value& data,
                                                            const Dictionary& schema_node,
                                                            const std::string& path) {
...
static std::optional<std::string> check_enum(const Value& data,
                                             const Dictionary& schema_node,
                                             const std::string& path) {
...
static std::optional<std::string> validate_node(const Value& data,
    ...
    if (path.empty() && schema_node.data.find("type") == schema_node.data.end() &&
        !schema_node.data.empty()) {
        // heuristic: if most entries are object specs, treat schema_node as properties
        bool looks_like_props = true;
        for (auto const& p : schema_node.items()) {
            if (!p.second.isDict()) {
                looks_like_props = false;
                break;
            }
        }
        if (looks_like_props) {
            ...
            if (!data.isDict())
                return std::optional<std::string>("expected object at top-level for schema");
            ...
            for (auto const& p : schema_node.items()) {
                ...
                if (itreq != propSpec.data.end() && itreq->second.isBool())
                    required = itreq->second.asBool();

                auto itObj = obj.data.find(propName);
                if (itObj == obj.data.end()) {
                    if (required)
                        return std::optional<std::string>("missing required property '" + propName +
                                                          "'");
                    continue;
                }
                ...
                if (auto dp = dict_ptr_from_value(childVal)) {
                    ...
                    if (auto err = validate_node(childVal, schema_root, propSpec, propName))
                        return err;
                }
            }
            return std::nullopt;
    ...
    if (it_allOf != schema_node.data.end() && it_allOf->second.isList()) {
        for (auto const& sub : it_allOf->second.asList()) {
            ...
            if (auto err = validate_node(data, schema_root, *subSchema, path)) {
                if (std::getenv("PS_VALIDATE_DEBUG")) {
                    std::cerr << "allOf subschema failed: data=" << data.dump() << " subschema="
                              << (subSchema ? subSchema->dump() : std::string("<null>"))
                              << " err=" << *err << "\n";
                }
                return err;
            }
    ...
    if (it_anyOf != schema_node.data.end() && it_anyOf->second.isList()) {
        ...
        if (!ok)
            return std::optional<std::string>("anyOf: no alternative matched for '" + path + "'");
    }
    ...
    if (it_oneOf != schema_node.data.end() && it_oneOf->second.isList()) {
        ...
        for (auto const& sub : it_oneOf->second.asList()) {
            ...
            if (!err.has_value())
                ...
                lastErrs.push_back(*err);
        }
        if (matched != 1)
            return std::optional<std::string>(
                        "oneOf: expected exactly one matching alternative for '" + path +
    ...
    if (it_type != schema_node.data.end() && it_type->second.isString()) {
        ...
        if (t == "object") {
            ...
            if (it_props != schema_node.data.end())
                props = schema_from_value(schema_root, it_props->second);
                        ...
                        if (propSchema) {
                            if (auto dp = dict_ptr_from_value(childVal)) {
                                if (auto err = validate_node(
                                                *dp, schema_root, *propSchema, childPath))
                                    return err;
                            } else {
                                if (auto err = validate_node(
                                                childVal, schema_root, *propSchema, childPath))
                                    return err;
                            }
                        }
                    }
                        ...
                        if (std::regex_match(od.first, re)) {
                            if (patSchema) {
                                std::string childPath =
                                            path.empty() ? od.first : (path + "." + od.first);
                                if (od.second.isDict()) {
                                    if (auto err = validate_node(*od.second.asDict(),
                                                                 schema_root,
                                                                 *patSchema,
                                                                 childPath))
                                        return err;
                                    ...
                                    if (auto err = validate_node(
                                                    od.second, schema_root, *patSchema, childPath))
                                        return err;
                                }
                            }
                        }
            ...
            if (it_add != schema_node.data.end()) {
                if (it_add->second.isBool())
                    allow_additional = it_add->second.asBool();
                else if (it_add->second.isDict()) {
                    allow_additional = true;  // additional properties allowed but must validate
                                              // against schema
            ...
            if (!allow_additional && props) {
                ...
                for (auto const& pd : obj.items()) {
                    ...
                    if (props->data.find(k) == props->data.end())
                        return std::optional<std::string>(
                                    "property '" + (path.empty() ? k : (path + "." + k)) +
                                    " not allowed by additionalProperties=false");
                }
            }
            // If additionalProperties is a schema, validate any keys not in properties or
            // patternProperties
            if (addPropsSchema) {
                ...
                if (it_pattern != schema_node.data.end() && it_pattern->second.isDict()) {
                    ...
                    for (auto const& ppent : pp.items()) {
                        try {
                            pattern_schemas.emplace_back(std::regex(ppent.first),
                                                         ppent.second.isDict()
                                                                     ? ppent.second.asDict().get()
                                                                     : nullptr);
                        } catch (...) {
                        }
                    }
                ...
                for (auto const& od : obj.items()) {
                    ...
                    if (od.second.isDict()) {
                        ...
                        if (auto err = validate_node(
                                        od.second, schema_root, *addPropsSchema, childPath))
                            return err;
                    }
                }
            }
            ...
            if (it_minp != schema_node.data.end() && it_minp->second.isInt()) {
                ...
                if (static_cast<int>(obj.size()) < minp)
                    return std::optional<std::string>("object '" + path +
                                                      "' has fewer properties than minProperties");
            }
            ...
            if (it_maxp != schema_node.data.end() && it_maxp->second.isInt()) {
                ...
                if (static_cast<int>(obj.size()) > maxp)
                    return std::optional<std::string>("object '" + path +
                                                      "' has more properties than maxProperties");
            }
                ...
                if (itemsVal.isList()) {
                    ...
                    for (size_t i = 0, n = arr.size(); i < n && i < schemas.size(); ++i) {
                        ...
                        if (itemSchema) {
                            ...
                            if (elem.isDict()) {
                                if (auto err = validate_node(*elem.asDict(),
                                                             schema_root,
                                                             *itemSchema,
                                                             childPath))
                                    return err;
                                ...
                                if (auto err = validate_node(
                                                elem, schema_root, *itemSchema, childPath))
                                    return err;
                            }
                        }
                    }
                    ...
                    if (arr.size() > schemas.size()) {
                        ...
                        if (it_add != schema_node.data.end()) {
                            if (it_add->second.isBool()) {
                                if (!it_add->second.asBool())
                                    return std::optional<std::string>(
                                                "array '" + path +
                                                "' has additional items but additionalItems=false");
                            } else if (it_add->second.isDict()) {
                                const Dictionary* addSchema =
                                            schema_from_value(schema_root, it_add->second);
                                if (addSchema) {
                                    for (size_t i = schemas.size(), n = arr.size(); i < n; ++i) {
                                        ...
                                        if (elem.isDict()) {
                                            ...
                                            if (auto err = validate_node(elem,
                                                                         schema_root,
                                                                         *addSchema,
                                                                         childPath))
                                                return err;
            ...
            if (it_unique != schema_node.data.end() && it_unique->second.isBool() &&
                it_unique->second.asBool()) {
                std::set<std::string> seen;
                for (auto const& el : arr) {
                    std::string s;
                    if (el.isDict())
                        s = el.asDict() ? el.asDict()->dump() : std::string("null");
                    else
                        s = el.dump();
                    if (seen.find(s) != seen.end())
                        return std::optional<std::string>("array '" + path +
                                                          "' contains non-unique items");
                    seen.insert(s);
                }
            }
            ...
            if (it_min != schema_node.data.end() && it_min->second.isInt()) {
                if (static_cast<int>(arr.size()) < it_min->second.asInt())
                    return std::optional<std::string>("array '" + path +
                                                      "' has fewer items than minItems");
            }
            auto it_max = schema_node.data.find("maxItems");
            if (it_max != schema_node.data.end() && it_max->second.isInt()) {
                if (static_cast<int>(arr.size()) > it_max->second.asInt())
                    return std::optional<std::string>("array '" + path +
                                                      "' has more items than maxItems");
            }
            return std::nullopt;
        } else if (t == "string") {
            if (!data.isString())
            ...
            if (it_minl != schema_node.data.end() && it_minl->second.isInt()) {
                if ((int)data.asString().size() < it_minl->second.asInt())
                    return std::optional<std::string>("property '" + path +
                                                      "' shorter than minLength");
            }
            auto it_maxl = schema_node.data.find("maxLength");
            if (it_maxl != schema_node.data.end() && it_maxl->second.isInt()) {
                if ((int)data.asString().size() > it_maxl->second.asInt())
                    return std::optional<std::string>("property '" + path +
            ...
            if (it_pattern != schema_node.data.end() && it_pattern->second.isString()) {
                ...
                try {
                    std::regex re(pat);
                    if (!std::regex_match(data.asString(), re))
                        return std::optional<std::string>("property '" + path +
            ...
            if (!(data.isInt() || (t == "number" && data.isDouble())))
                return std::optional<std::string>(limit_line_length(
                            "expected type '" + t + "' at '" + display_path(path) +
...
std::optional<std::string> validate(const Dictionary& data, const Dictionary& schema) {
    ...
    if (data.scalar) {
        ...
        if (std::getenv("PS_VALIDATE_DEBUG")) {
            std::cerr << "validate debug: data=" << data.scalar->dump()
                      << " schema=" << schema.dump() << " result=" << (res ? *res : "<ok>") << "\n";
        }
        return res;
    } else {
